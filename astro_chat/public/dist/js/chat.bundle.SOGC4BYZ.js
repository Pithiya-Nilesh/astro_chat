(() => {
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_bubble.js
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_utils.js
  ...
  async function get_rooms(email) {
    const res = await frappe.call({
      type: "GET",
      method: "astro_chat.api.room.get",
      args: {
        email
      }
    });
  ...
  async function get_messages(room, email) {
    const res = await frappe.call({
      method: "astro_chat.api.message.get_all",
      args: {
        room,
        email
      }
  ...
  async function send_message(content, user, room, email) {
    try {
      await frappe.call({
        method: "astro_chat.api.message.send",
        args: {
          content,
          user,
          room,
  ...
  async function mark_message_read(room) {
    try {
      await frappe.call({
        method: "astro_chat.api.message.mark_as_read",
        args: {
          room
        }
      });
  ...
  async function create_guest({ email, full_name, message }) {
    const res = await frappe.call({
      method: "astro_chat.api.user.get_guest_room",
      args: {
        email,
        full_name,
        message
  ...
  async function set_typing(room, user, is_typing, is_guest) {
    try {
      await frappe.call({
        method: "astro_chat.api.message.set_typing",
        args: {
          room,
          user,
          is_typing,
  ...
  async function create_private_room(room_name, users, type) {
    await frappe.call({
      method: "astro_chat.api.room.create_private",
      args: {
        room_name,
        users,
        type
  ...
  async function set_user_settings(settings) {
    await frappe.call({
      method: "astro_chat.api.config.user_settings",
      args: {
        settings
      }
    });
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_space.js
  var ChatSpace = class {
    constructor(opts) {
    ...
    setup_header() {
      this.avatar_html = get_avatar_html(this.profile.room_type, this.profile.opposite_person_email, this.profile.room_name);
      const header_html = `
    ...
    make_messages_html(messages_list) {
      ...
      if (this.profile.message) {
        ...
        send_message(this.profile.message.content, this.profile.user, this.profile.room, this.profile.user_email);
      }
      messages_list.forEach((element) => {
        ...
        this.message_html += this.make_message(element.content, get_time(element.creation), message_type, element.sender).prop("outerHTML");
    ...
    setup_events() {
      ...
      $(".type-message").keydown(function(e) {
        if (e.which === 13) {
          ...
          if (me.typing === false) {
            me.typing = true;
            set_typing(me.profile.room, me.profile.user_email, me.typing, !me.profile.is_admin);
            me.timeout = setTimeout(me.typing_timeout, 3e3);
          }
        }
      });
    }
    setup_socketio() {
      console.log("setup_socketio start in space");
      console.log("realtime", this.profile.room);
      const me = this;
      frappe.realtime.on(this.profile.room, function(res) {
        console.log("setup_socketio1", res);
    ...
    handle_send_message(attachment) {
      ...
      this.$chat_space_container.append(this.make_message(content, get_time(), "recipient", this.profile.user));
      $type_message.val("");
      scroll_to_bottom(this.$chat_space_container);
      send_message(content, this.profile.user, this.profile.room, this.profile.user_email);
    }
    receive_message(res, time) {
      ...
      this.$chat_space_container.append(this.make_message(res.content, time, chat_type, res.user));
      scroll_to_bottom(this.$chat_space_container);
    }
    render() {
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_room.js
  var ChatRoom = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_add_room.js
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_user_settings.js
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_list.js
  var ChatList = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
      this.user = opts.user;
    ...
    setup_socketio() {
      console.log("setup_socketio start in list");
      const me = this;
      frappe.realtime.on("latest_chat_updates", function(res) {
        console.log("setup_socketio", res);
        const chat_room_item = me.chat_rooms.find((element) => element[0] === res.room);
        ...
        console.log("adfasdf", message);
        console.log("res df", res.creation);
        if ($(".chat-list").length) {
      ...
      frappe.realtime.on("new_room_creation", function(res) {
        console.log("new room creation", res);
        if (!$(".chat-element").is(":visible") && frappe.Chat.settings.user.enable_notifications === 1) {
          frappe.utils.play_sound("chat-notification");
        }
        res.user = me.user;
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_form.js
  var ChatForm = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
      this.profile = opts.profile;
  ...
  // ../astro_chat/astro_chat/public/js/components/chat_welcome_screen.js
  var ChatWelcome = class {
    ...
    setup() {
      this.$chat_welcome_screen = $(document.createElement("div")).addClass("chat-welcome");
      const welcome_html = `
  ...
  // ../astro_chat/astro_chat/public/js/chat.bundle.js
...
//! momentjs.com
//! version : 2.29.4
//# sourceMappingURL=chat.bundle.SOGC4BYZ.js.map
